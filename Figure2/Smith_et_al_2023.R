...
#combine clusters, then show enriched in inflammed sublining

rna$clus_combined <- rna@meta.data[["subtype_fine"]]
Idents(rna) <- 'clus_combined'
levels(rna)
current.sample.ids <- levels(rna)
new.sample.ids <- c("SL", "SL","LL","LL", "Intemdiate LL/SS", "Intemdiate LL/SS")

rna@meta.data[["clus_combined"]] <- plyr::mapvalues(x = rna@meta.data[["clus_combined"]], from = current.sample.ids, to = new.sample.ids)
